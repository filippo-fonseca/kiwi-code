...
New Kiwi-only code (no upstream conflict surface): `packages/contracts/src/bgsd.ts`, `apps/server/src/bgsd/` (discovery, bridgeClient, fsWatcher, projection, service + tests), `apps/web/src/kiwi/` (feed, bgsdStore, kiwiTheme, dashboard components, status bar, sidebar section + tests).
